...
from tinker_cookbook.completers import TinkerTokenCompleter, DebugTokenCompleter, TwoPhaseTokenCompleter
...
def _write_trajectory_logs(
    ...
    with open(out_path, "a") as f:
        for tags, trajectory_group in zip(taglist_P, trajectory_groups_P, strict=True):
            for traj_idx, (traj, final_reward, metrics) in enumerate(
                zip(
                ...
                rewards = [transition.reward for transition in traj.transitions]
                steps = []
                for step_idx, transition in enumerate(traj.transitions):
                    ...
                    steps.append(
                        {
                            "step": step_idx,
                            "ob_text": tokenizer.decode(ob_tokens),
                            "ac_text": tokenizer.decode(transition.ac.tokens),
                ...
                payload: dict[str, Any] = {
                    ...
                    "total_reward": sum(rewards) + final_reward,
                    "step_rewards": rewards,
                    "steps": steps,
                    "final_ob_text": tokenizer.decode(list(traj.final_ob.to_ints())),
                }
                if train_step is not None:
                    payload["train_step"] = train_step
                if metrics:
                    payload["trajectory_metrics"] = metrics
                step_metrics = [
                    transition.metrics for transition in traj.transitions if transition.metrics
                ]
                if step_metrics:
...
@scope
def print_group(traj_group: TrajectoryGroup, tokenizer: Tokenizer, adv_estimator: str="mean_baseline", adv_estimator_beta: float = 2.0):
    """
    ...
    advantages_G = compute_advantages([traj_group], adv_estimator, adv_estimator_beta=adv_estimator_beta)
...
class Config:
    ...
    # Debug mode - skip LLM, use placeholder programs
        ...
        with _get_logtree_scope(
            ...
            async def trajectory_group_worker_task(
                ...
                trajectory_group = await do_group_rollout_and_filter_constant_reward(
                    ...
                    step_idx=i_batch,
                    debug_skip_llm=cfg.debug_skip_llm,
...
async def do_group_rollout_and_filter_constant_reward(
    ...
    if debug_skip_llm:
        policy = DebugTokenCompleter(tokenizer)
    elif two_phase_sampling:
        ...
        policy = TinkerTokenCompleter(sampling_client, max_tokens=max_tokens, temperature=temperature, dynamic_max_tokens=dynamic_max_tokens)
...
async def prepare_minibatch(
    ...
    for traj_group in trajectory_groups_P[:2]:
        print_group(traj_group, tokenizer, adv_estimator, adv_estimator_beta=adv_estimator_beta)
...
async def skip_train_step(
    ...
    metrics = {}
    _data_D, prepare_minibatch_metrics = await prepare_minibatch(
        env_group_builders_P,
        trajectory_groups_P,
        tokenizer,
        service_client,
    ...
    sampling_client, checkpoint_metrics = await save_checkpoint_and_get_sampling_client(
        training_client, i_batch, cfg.log_path, cfg.save_every
    )
    metrics.update(checkpoint_metrics)
...
async def do_sync_training(
    ...
    for i_batch in range(start_batch, end_batch):
        ...
        if cfg.env == "ac1":
            from tasks.alphaevolve_ac.best_sequence_utils import get_best_bound_path, clear_step_entry
        ...
        if cfg.eval_every > 0 and i_batch % cfg.eval_every == 0:
            with timed("run_evals", metrics):
                ...
                if 'test/table' in eval_metrics:
                    table_data = eval_metrics.pop('test/table')
                    scores = [row[2] for row in table_data]
                    mean_score = np.mean(scores) if scores else 0
                    table_data = [(*row, row[2] - mean_score) for row in table_data]
                    test_table = {
        ...
        # TODO: I am unsure if the train step will remake the prompt, which could cause a mismatch from rollout
        # if we are not setting randomness, may need to guarantee the same prompt/states.
        if not cfg.eval_BoN:
            # Train step
            sampling_client, train_step_metrics = await do_train_step_and_get_sampling_client(
                cfg,
            ...
            # No update to training_client, but save ckpt to resume continuously
            sampling_client, train_step_metrics = await skip_train_step(
                cfg,
                i_batch,
                training_client,
                service_client,
...
async def main(
    ...
    if cfg.async_config is not None:
        assert not cfg.eval_BoN, "Eval BoN not supported in `do_async_training`"
        training_func = do_async_training
    elif cfg.stream_minibatch_config is not None:
        assert not cfg.eval_BoN, "Eval BoN not supported in `do_sync_training_with_stream_minibatch`"
